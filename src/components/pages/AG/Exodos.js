...
function Exodo(exodo) {
  exodo = exodo.exodo
  return (
    <Col xl={3} lg={4} md={6} sm={12}>
      <Card className='my-2'>
        <Card.Img style={{width: "125px",height: '125px',display: "block",marginLeft: "auto",marginRight: "auto"}} variant="top" src={exodo.logo} alt='No carga porque ps no hay nada'/>
        <Card.Body>
          <Card.Title>{exodo.nombre}</Card.Title>
          <Card.Text><b>Lema: </b>{exodo.lema}</Card.Text>
          <Card.Text><b>Tribus: </b>{exodo.tribus}</Card.Text>
          <Card.Text><b>Porra: </b>{exodo.porra}</Card.Text>
          <Card.Text><b>Fotos: </b>{exodo.fotos}</Card.Text>
          <Card.Text><b>Video: </b>{exodo.video}</Card.Text>
        </Card.Body>
      </Card>
    </Col>
  )
  ...
  render() {
    return (
      <div className='exodos'>
        <CardGroup>
          { this.state.exodos.map((exodo, index) => (<Exodo exodo={exodo} key={index}></Exodo>))}
        </CardGroup>
      </div>
    )
  }
