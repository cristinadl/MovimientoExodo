import React, { Component } from 'react'
import { Link } from 'react-router-dom'
import Card from 'react-bootstrap/Card'
import CardGroup from 'react-bootstrap/CardGroup'
import { Container, Row, Col } from 'reactstrap'
import Jumbotron from 'react-bootstrap/Jumbotron'
import Image from 'react-bootstrap/Image'

import * as firebase from 'firebase'

var db;


function Exodo(props) {
  const exodo = props.exodo
  return (
    <Col xl={3} lg={4} md={6} sm={12}>
      <Card onClick={props.selectExodo} className='my-2'>
        <Card.Img style={{ width: "125px", height: '125px', display: "block", marginLeft: "auto", marginRight: "auto", marginTop: "5px" }} variant="top" src={exodo.logo} alt='No carga porque ps no hay nada' />
        <Card.Body>
          <Card.Title>{exodo.nombre}</Card.Title>

        </Card.Body>
      </Card>
    </Col>
  )
}

function ExodoData(props) {
  const exodo = props.exodo
  return (
    <Col >
      <Card onClick={props.selectExodo} className='my-2'>
        <Jumbotron style={jumbotronStyle}>
          <br></br>
          <h1 style={textColor}>{exodo.nombre}</h1>
          <p>{lema}</p>
        </Jumbotron>
        <Jumbotron style={jumbotronStyle2}>
          <Container>
            <Row>
              <Col>
                <Image src={exodo.logo} thumbnail rounded />
                <h3>{exodo.nombre}</h3>
              </Col>
              <Col xs={8} style={{ textAlign: 'left' }}>
                <h4>NUESTRA PORRA</h4>
                <p style={textColor}>{porra}</p>
                <h4>NUESTRO LEMA</h4>
                <p style={textColor}>{lema}</p>
              </Col>
            </Row>
            <hr></hr>
            <Row>
              <Col>
                <h4>HISTORIA</h4>
                <p style={textColor}>{historia}</p>
                <hr></hr>
                <h4>¿SABÍAS QUE?</h4>
                <p style={textColor}>{sabiasQue}</p>
                <p style={textColor}> ·Los colores oficiales del grupo son Blanco, Rojo, Anaranjado, Gris, Negro y Crema ya que son los colores representativos de nuestra sede actual.</p>
                <p style={textColor}> ·El primer logo del grupo estaba representada por una T taus, que es el simbolo representativo de los Franciscanos.</p>
                <p style={textColor}> ·Todas las tribus tienen 3 rangos dentro de Exoditos Nombrados, más jefe y subjefe nombrado, en total cada tribu tiene 5 pañoletas representativas.</p>
                <p style={textColor}> ·La pañoleta de Jefe Nombrado es el color representativo de la tribu con doble filo negro con un espacio entre estos.</p>
              </Col>
            </Row>
          </Container>
        </Jumbotron>
      </Card>
    </Col>
  )
}

export default class Exodos extends Component {
  constructor(props) {
    super(props);
    this.state = {
      exodos: [],
      internacional: [],
      local: [],
      exodo_id: null,
      loading: true
    };
    this.selectExodo = this.selectExodo.bind(this)
    this.resetExodo = this.resetExodo.bind(this)
  }

  componentDidMount() {
    var exodos = this.state.exodos
    const logo = 'data:image/jpeg;base64,'
    db = firebase.firestore();
    db.collection('Usuarios')
      .where('tipoExodo', '==', true)
      .get()
      .then((Snap) => {
        Snap.forEach(function (exodo) {
          exodos.push({
            id: exodo.ref.id,
            nombre: exodo.data().nombre,
            internacional: exodo.data().Internacional,
            logo: logo
          });
        });
        var internacional = exodos.filter(function (exodo) { return exodo.internacional })
        var local = exodos.filter(function (exodo) { return !exodo.internacional })
        this.setState({ exodos: exodos, internacional: internacional, local: local, loading: false })
      });
  }

  selectExodo(id) {
    console.log('update')
    console.log(id)
    this.setState(
      { exodo_id: id }
    )
  }

  resetExodo() {
    this.updateState(null)
  }

  render() {
    const plus_sign = 'https://cdn.pixabay.com/photo/2014/04/02/10/55/plus-304947_960_720.png'
    var current_id = this.state.exodo_id;
    return (
      <div className='exodos' ref={this.exodos}>
        {this.state.exodo_id ?
          <ExodoData exodo={this.state.exodos.filter(function (exodo) { return exodo.id == current_id })[0]}></ExodoData>
          :
          this.state.loading ? <div className='loader center' /> :
            <div>
              <Card>
                <Card.Body>
                  <Card.Title> Internacional </Card.Title>
                  <CardGroup>{this.state.internacional.map((exodo, index) => (<Exodo className='exodo' exodo={exodo} key={exodo.id} selectExodo={() => this.selectExodo(exodo.id)}></Exodo>))}</CardGroup>
                </Card.Body>
              </Card>
              <Card>
                <Card.Body>
                  <Card.Title>Local</Card.Title>
                  <CardGroup>{this.state.local.map((exodo, index) => (<Exodo className='exodo' exodo={exodo} key={exodo.id} selectExodo={() => this.selectExodo(exodo.id)}></Exodo>))}</CardGroup>
                </Card.Body>
              </Card>
            </div>

        }
      </div>
    )
  }
}

const textColor = {
  color: "#445cbc",
}
const porra = "Aquí está la barra loca de los desiertos! Siempre aquí y allá yo voy descalzo. Ya que daría la vida por los colores, siempre en cuerpo y alma con Jesús Cristo... Para ser mejores!!! [...] Esto es Cadés! Hay que estar corriendo con fortaleza! Esto es Cadés! Esto es de la mano con Jesús Cristo!"
const lema = "Aquí está la barra loca de los desiertos!"
const historia = "Grupo Éxodo Cadés con espiritualidad diocesana, tiendo como fecha de fundación el día 02 de Febrero del 2013, el cual pertenece a la Pastoral de la Parroquia de Nuestra Señora del Camino ubicada en la dirección de Vía Atlixcayotl No. 6402, Colonia Concepción La Cruz, Puebla, Pue. Código Postal 72197. En la larga busqueda por darse a conocer iniciamos actividades en la Capilla Real de San Pedro Apostol ubicado en San Pedro Cholula, como ya existia un grupo adolescente en dicha sede a nuestro grupo se le complico el realizar atividades por lo cual se cambia de sede a la actual apartir del 01 de Junio del 2013."
const sabiasQue = "·La primera tribu fundada en el grupo fue la tribu de Simeón"

const jumbotronStyle2 = {
  padding: '1rem',
  background: 'white'
}

const jumbotronStyle = {
  background: 'url(http://movimientoexodo.com/wp-content/uploads/2015/01/dd23aaa.jpg) no-repeat 20% 25%',
  backgroundSize: "cover",
  textAlign: 'right',
}